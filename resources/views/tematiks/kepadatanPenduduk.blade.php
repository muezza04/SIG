...
@section('title', 'Kepadatan Penduduk')
        ...
        fetch('/data/wilayah.json')
            ...
            .then(wilayahData => {
                fetch('/data/tematik.json') // Ambil tematik.json
                    .then(response => response.json())
                    .then(tematikData => {
                        const kepadatanPendudukData = tematikData.data.kepadatan_penduduk.features;
                        ...
                        L.geoJSON(wilayahData, {
                            ...
                            onEachFeature: function (feature, layer) {
                                ...
                                const dataKecamatan = kepadatanPendudukData.find(item => item.kecamatan === kecamatanName);

                                // Bind data ke popup
                                const kepadatan_pendudukWil = dataKecamatan ? dataKecamatan.kepadatan_penduduk : "Data tidak tersedia";
                                const popupContent = `
                                    <b>${kecamatanName}</b><br>
                                    Kepadatan Penduduk: ${kepadatan_pendudukWil} ${tematikData.data.kepadatan_penduduk.unit}
                                `;
...
<h1 class="text-2xl font-bold mt-4 hover:text-blue-500"><a href="">Peta Tematik Kepadatan Penduduk</a></h1>
<p class="mt-4 mb-8 text-lg text-gray-600 text-justify" >{{ $tematik['data']['kepadatan_penduduk']['desk'] }}</p>
    ...
    <table class="table-auto border-collapse border border-blue-500 text-center w-4/5">
        <thead class="border border-blue-500 bg-blue-100">
            <tr>
                ...
                <th class="w-1/2 border border-blue-500 py-2">Kepadatan ({{ $tematik['data']['kepadatan_penduduk']['unit'] }})</th>
            </tr>
        </thead>
        <tbody class="border border-blue-500">
            @foreach($tematik['data']['kepadatan_penduduk']['features'] as $feature)
            <tr class="{{ $loop->even ? 'bg-blue-200' : '' }}">
                <td class="border border-blue-500 py-1">{{ $feature['kecamatan'] }}</td>
                <td class="border border-blue-500 py-1">{{ number_format($feature['kepadatan_penduduk'], 3) }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
