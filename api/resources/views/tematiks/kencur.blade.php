        ...
        fetch('/data/wilayah.json')
            ...
            .then(wilayahData => {
                fetch('/data/tematik.json') // Ambil tematik.json
                    .then(response => response.json())
                    .then(tematikData => {
                        const kencurData = tematikData.data.produksi_biofarmaka.features;
                        ...
                        L.geoJSON(wilayahData, {
                            ...
                            onEachFeature: function (feature, layer) {
                                ...
                                const dataKecamatan = kencurData.find(item => item.kecamatan === kecamatanName);

                                // Bind data ke popup
                                const produksi_biofarmakaWil = dataKecamatan ? dataKecamatan.kencur : "Data tidak tersedia";
                                const popupContent = `
                                    <b>${kecamatanName}</b><br>
                                    Kencur: ${produksi_biofarmakaWil} ${tematikData.data.produksi_biofarmaka.unit}
                                `;
...
<h1 class="text-2xl font-bold mt-4 hover:text-blue-500"><a href="">Peta Tematik Produk Kencur</a></h1>
<p class="mt-4 mb-8 text-lg text-gray-600 text-justify" >{{ $tematik['data']['produksi_biofarmaka']['desk_kencur'] }}</p>
    ...
    <table class="table-auto border-collapse border border-blue-500 text-center w-4/5">
        <thead class="border border-blue-500 bg-blue-100">
            <tr>
                ...
                <th class="w-1/2 border border-blue-500 py-2">Kencur ({{ $tematik['data']['produksi_biofarmaka']['unit'] }})</th>
            </tr>
        </thead>
        <tbody class="border border-blue-500">
            ...
            <tr class="{{ $loop->even ? 'bg-blue-200' : '' }}">
                ...
                <td class="border border-blue-500 py-1">{{ number_format($feature['kencur'], 0) }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
