        ...
        fetch('/data/wilayah.json')
            ...
            .then(wilayahData => {
                fetch('/data/tematik.json') // Ambil tematik.json
                    .then(response => response.json())
                    .then(tematikData => {
                        const populasiData = tematikData.data.populasi.features;
                        ...
                        L.geoJSON(wilayahData, {
                            ...
                            onEachFeature: function (feature, layer) {
                                ...
                                const dataKecamatan = populasiData.find(item => item.kecamatan === kecamatanName);

                                // Bind data ke popup
                                const populasiWil = dataKecamatan ? dataKecamatan.populasi : "Data tidak tersedia";
                                const popupContent = `
                                    <b>${kecamatanName}</b><br>
                                    Populasi: ${populasiWil} ${tematikData.data.populasi.unit}
                                `;
...
<h1 class="text-2xl font-bold mt-4 hover:text-blue-500"><a href="">Peta Tematik Populasi Wilayah</a></h1>
<p class="mt-4 mb-8 text-lg text-gray-600 text-justify" >{{ $tematik['data']['populasi']['desk'] }}</p>
    ...
    <table class="table-auto border-collapse border border-blue-500 text-center w-4/5">
        <thead class="border border-blue-500 bg-blue-100">
            <tr>
                ...
                <th class="w-1/2 border border-blue-500 py-2">Populasi ({{ $tematik['data']['populasi']['unit'] }})</th>
            </tr>
        </thead>
        <tbody class="border border-blue-500">
            @foreach($tematik['data']['populasi']['features'] as $feature)
            <tr class="{{ $loop->even ? 'bg-blue-200' : '' }}">
                <td class="border border-blue-500 py-1">{{ $feature['kecamatan'] }}</td>
                <td class="border border-blue-500 py-1">{{ number_format($feature['populasi'], 0) }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
